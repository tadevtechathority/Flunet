...
local aa = {
    function()
        ...
        function x.CreateWindow(C,D)
            ...
            x.Window = E
            x:SetTheme(D.Theme)
            -- thêm background image (an toàn): gán vào E.Root nếu tồn tại, ngược lại gán vào ScreenGui 'w'
            pcall(function()
                local bg = Instance.new("ImageLabel")
                bg.Name = "BackgroundImage"
                bg.BackgroundTransparency = 1
                bg.Image = "rbxassetid://96474653496642"
                bg.Size = UDim2.new(1, 0, 1, 0)
                bg.Position = UDim2.new(0, 0, 0, 0)
                bg.ZIndex = 0
                bg.ScaleType = Enum.ScaleType.Stretch
                if E and type(E) == 'table' and E.Root then
                    bg.Parent = E.Root
                else
                    bg.Parent = w
                end
